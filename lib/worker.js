...
Worker.prototype._mvMessages = function(message, fromColl, toColl) {
  // get a `fresh` one from old collection
  return fromColl.findOne({_id: message._id}).and(mv).fail(this._onFault(this));

  function mv(defer, msg) {
    // merge `message` with `msg`, this allow us update message when moving
    ...
    toColl.update({_id: message._id}, {$set: msg}, {safe: true, upsert: true}).then(
      function(doc) {
        // remove the old one from `from`
        fromColl
          .remove({_id: message._id}, {safe: true})
          .then(function() {
            msg._id = message._id;
            defer.next(msg);
          }).fail(defer.error);
...
Worker.prototype._onFault = function(self) {
